...
export const FIRST_MEGA_ROOM_ZONES = Object.freeze([
  ...
  zone("S06", "동부 승강축", "transition", "tall", 18, 2, "첫 수직 이동과 카메라 추종", "빠른 승강기와 느린 발판 계단 중 선택", "승강기 아래 고정 안전대와 즉시 재호출 장치", "상층에서 절단 권한의 봉인 신호가 감지된다.", "없음", { checkpoint: true }),
  ...
  zone("S08", "절단 권한실", "skill", "wide", 18, 1, "기본 공격 권한 회수와 정지 표적", "가까운 표적과 높은 표적 중 먼저 시험", "공격하지 않아도 출구가 막히지 않는 안전실", "관리체를 손상하지 않으려 봉인했던 절단 권한을 회수한다.", "고정형 무해 표적", { unlock: "basic_attack" }),
  ...
  zone("S19", "공명 날개 제단", "skill", "junction", 12, 1, "이중 점프 기능 기억 회수", "즉시 시험 우물로 가거나 기록을 확인", "능력 획득 방에는 피해와 낙사 없음", "공명 날개는 새 장비가 아니라 잃어버린 움직임이다.", "없음", { unlock: "double_jump" }),
  zone("S20", "이중 도약 우물", "navigation", "tall", 16, 2, "첫 점프 정점에서 두 번째 점프 사용", "넓은 중간 발판과 높은 직행 발판 중 선택", "실패하면 우물 바닥에서 즉시 다시 시도", "복원된 기억이 몸의 공명 출력을 두 단계로 나눈다."),
  zone("S21", "상하 분기 공방", "choice", "junction", 20, 3, "이중 점프를 이용한 두 경로 비교", "아래 안전 전투 길과 위쪽 빠른 정밀 길 중 선택", "두 길 모두 중앙 회수 공간으로 연결", "작업자 기록과 능력 조각 중 무엇을 먼저 볼지 선택한다.", "아래 길 크롤러 1체"),
  ...
  zone("S24", "서부 수직 회수로", "transition", "tall", 20, 2, "붕괴 구간 뒤 안전한 수직 회복", "승강추를 작동하거나 이중 점프로 측면 발판 상승", "중간 휴식대 세 개와 낙하 완충 바닥", "위층에 마지막 철수자의 기억 신호가 남아 있다.", "없음", { checkpoint: true }),
  ...
  zone("S31", "고요한 근원 전실", "story", "wide", 12, 1, "보스 전 규칙과 약점 예고", "연습 표적에서 공격하거나 바로 진입", "보스방과 분리된 안전 준비 공간", "수호자는 코어리스를 기억하지만 침입자로 판정한다."),
  zone("S32", "수호자 외곽 고리", "combat", "arena", 28, 3, "보스의 두 핵심 공격을 약한 형태로 학습", "낮은 엄폐와 높은 이중 점프 회피 중 선택", "피해 뒤 넓은 회복 창과 퇴장 가능한 입구", "외곽 방어체가 수호자의 공격 리듬을 전달한다.", "외곽 방어체 2체"),
  zone("S33", "제련 수호자 1막", "boss", "arena", 36, 5, "돌진·내려찍기 뒤 반격 창", "지상 회피와 이중 점프 회피를 상황에 맞게 선택", "각 패턴 뒤 최소 1.2초 안전 반격 창", "수호자는 코어리스를 원점 폭주의 원인으로 기억한다.", "제련 수호자"),
...
export const MEGA_ROOM_CAMPAIGN = Object.freeze([
  megaRoom(1, 1, "빈 제련소", ["절단 권한", "공명 날개"], 1, "제련 수호자", "코어리스가 깨어나 원점 코어의 신호를 처음 추적한다."),
  megaRoom(2, 1, "냉각 수로", [], 1, "수문 탈출", "검은 공명이 시설 전체로 흐르고 있음을 확인한다."),
...
export function validatePass01ScaleLayout() {
  ...
  const checks = [
    ...
    ["attackBeforeMandatoryCombat", FIRST_MEGA_ROOM_ZONES.findIndex(item => item.unlock === "basic_attack") < FIRST_MEGA_ROOM_ZONES.findIndex(item => item.category === "combat")],
    ["doubleJumpBeforeItsTests", FIRST_MEGA_ROOM_ZONES.findIndex(item => item.unlock === "double_jump") < FIRST_MEGA_ROOM_ZONES.findIndex(item => item.id === "S20")],
