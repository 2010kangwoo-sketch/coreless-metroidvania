...
const samples = await page.evaluate(() => window.__corelessV2.pass38.cameraSamples);
const sampleMetrics = [];
for (const sample of samples) {
  const metrics = await page.evaluate(sampleData => {
    const runtime = window.__corelessV2.runtime;
    const pass38 = window.__corelessV2.pass38;
    ...
    const scene = pass38.scenes.find(item => item.id === sampleData.sceneId);
    ...
    const continuationBottoms = pass38.placements.filter(item => item.sceneId === scene.id && item.continuesBelowViewport).map(screenRect).map(rect => ({ id: rect.id, bottom: rect.y + rect.height }));
  ...
  await page.locator("#gameCanvas").screenshot({ path: `browser-artifacts/pass38-sweep-${sample.id}.png` });
}

const assetBottomCoverage = await page.evaluate(() => {
  const result = {};
  for (const id of ["precision_entry", "precision_return", "curve_upper", "curve_drop", "curve_lower"]) {
    const record = window.__corelessV2.pass38.assetState.byId.get(id);
...
const parallaxProbe = await page.evaluate(() => {
  const pass38 = window.__corelessV2.pass38;
  const scene = pass38.scenes[0];
  const far = pass38.placements.find(item => item.sceneId === scene.id && item.layer === "far_background");
  const foreground = pass38.placements.find(item => item.sceneId === scene.id && item.layer === "foreground");
...
await page.locator("#gameCanvas").screenshot({ path: "browser-artifacts/pass38-blueprint-focused.png" });
await page.keyboard.press("b");

const resultData = await page.evaluate(() => ({ title: document.title, audit: window.__corelessV2.audit(), validation: window.__corelessV2.pass38.validate(), plan: window.__corelessV2.pass38.plan, assetState: { loadedCount: window.__corelessV2.pass38.assetState.loadedCount, failedCount: window.__corelessV2.pass38.assetState.failedCount, dimensionsValid: window.__corelessV2.pass38.assetState.dimensionsValid }, debug: window.__corelessV2.debug() }));
...
const checks = {
  title: resultData.title === "Coreless · Rebuild V2 · Pass 38",
  runtimeAudit: resultData.audit.passed && resultData.audit.passedCount === 85,
  pass38Validation: resultData.validation.passed && resultData.validation.passedCount === 45,
  explicitScope: resultData.plan.scope === "precision_parkour_and_giant_direction_turn_only",
  ...
  fiveScenesCovered: new Set(sampleMetrics.map(item => item.sceneId)).size === 5,
  ...
  colossalOverviewComposite: resultData.plan.sceneCount === 5 && resultData.plan.placementCount === 22,
...
const result = { version: "rebuild-v2-pass38-focused", category: "precision parkour and colossal direction-turn raster depth across twelve helper-positioned camera samples; helper movement is not a traversal claim", helperCoordinateMovement: true, passed: Object.values(checks).every(Boolean), checks, measurements: { minimumContrast, maximumFlatCoverage, maximumMagentaLeakCoverage, minimumContinuationBottom, assetBottomCoverage, parallaxProbe, sampleMetrics }, resultData, consoleErrors, pageErrors };
fs.writeFileSync("browser-artifacts/pass38-focused-results.json", JSON.stringify(result, null, 2));
console.log(JSON.stringify({ passed: result.passed, checks, measurements: result.measurements, audit: `${resultData.audit.passedCount}/${resultData.audit.total}`, pass38: `${resultData.validation.passedCount}/${resultData.validation.total}` }, null, 2));
await browser.close();
server.kill("SIGTERM");
