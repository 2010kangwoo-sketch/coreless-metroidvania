...
const server = spawn("python3", ["-m", "http.server", "4230"], { cwd: process.cwd(), stdio: "ignore" });
...
await page.goto("http://127.0.0.1:4230/index.html", { waitUntil: "networkidle" });
...
const sampleMetrics = [];
const samples = await page.evaluate(() => window.__corelessV2.pass35.cameraSamples);
for (const sample of samples) {
  const metrics = await page.evaluate(sampleData => {
    const runtime = window.__corelessV2.runtime;
    const pass35 = window.__corelessV2.pass35;
    runtime.progress.pass06Completed = true;
    runtime.progress.zone06Entered = true;
    runtime.progress.curveCommitted = true;
    runtime.progress.zone06Dropped = sampleData.id !== "curve_entry" && sampleData.id !== "curve_apex";
    ...
    const scene = pass35.scenes.find(item => item.id === sampleData.sceneId);
    const safety = { x: Math.max(0, screenPlayerX - 42), y: Math.max(0, screenPlayerY - 42), width: 118, height: 126 };
    ...
    const activeForeground = pass35.placements.filter(item => item.sceneId === scene.id && item.layer === "foreground" && (item.minCameraX === undefined || runtime.camera.x >= item.minCameraX) && (item.maxCameraX === undefined || runtime.camera.x <= item.maxCameraX));
    const foregroundOverlapIds = [];
    for (const placement of activeForeground) {
      const rect = screenRect(placement);
      const record = pass35.assetState.byId.get(placement.assetId);
    ...
    const continuationBottoms = pass35.placements.filter(item => item.sceneId === scene.id && item.continuesBelowViewport).map(screenRect).map(rect => ({ id: rect.id, bottom: rect.y + rect.height }));
  ...
  await page.locator("#gameCanvas").screenshot({ path: `browser-artifacts/pass35-sweep-${sample.id}.png` });
}

const assetBottomCoverage = await page.evaluate(() => {
  const result = {};
  for (const id of ["curve_chamber", "east_dash_vault", "middle_dash_vault", "west_dash_vault", "wave_gallery", "exit_vault"]) {
    const record = window.__corelessV2.pass35.assetState.byId.get(id);
...
const parallaxProbe = await page.evaluate(() => {
  const pass35 = window.__corelessV2.pass35;
  const scene = pass35.scenes[0];
  const far = pass35.placements.find(item => item.sceneId === scene.id && item.layer === "far_background");
  const foreground = pass35.placements.find(item => item.sceneId === scene.id && item.layer === "foreground");
...
await page.locator("#gameCanvas").screenshot({ path: "browser-artifacts/pass35-blueprint-focused.png" });
await page.keyboard.press("b");

const resultData = await page.evaluate(() => ({ title: document.title, audit: window.__corelessV2.audit(), validation: window.__corelessV2.pass35.validate(), plan: window.__corelessV2.pass35.plan, assetState: { loadedCount: window.__corelessV2.pass35.assetState.loadedCount, failedCount: window.__corelessV2.pass35.assetState.failedCount, dimensionsValid: window.__corelessV2.pass35.assetState.dimensionsValid }, debug: window.__corelessV2.debug() }));
...
const checks = {
  title: resultData.title === "Coreless · Rebuild V2 · Pass 35",
  runtimeAudit: resultData.audit.passed && resultData.audit.passedCount === 70,
  pass35Validation: resultData.validation.passed && resultData.validation.passedCount === 50,
  explicitScope: resultData.plan.scope === "giant_curve_and_dash_run_only",
  ...
  farParallaxSlower: Math.abs(parallaxProbe.farDelta - 6) < 0.01,
  foregroundParallaxFaster: Math.abs(parallaxProbe.foregroundDelta - 107) < 0.01,
  ...
  dashGapsRasterized: resultData.plan.dashGapSpriteCount === 3 && resultData.plan.dashGapAssetId === "dash_gap_ends",
...
const result = { version: "rebuild-v2-pass35-focused", category: "giant curve and dash-run raster depth across twelve helper-positioned camera samples; helper movement is not a traversal claim", helperCoordinateMovement: true, passed: Object.values(checks).every(Boolean), checks, measurements: { minimumContrast, maximumFlatCoverage, totalVisibleSeamColumns, foregroundOverlapCount, minimumContinuationBottom, assetBottomCoverage, parallaxProbe, sampleMetrics }, resultData, consoleErrors, pageErrors };
fs.writeFileSync("browser-artifacts/pass35-focused-results.json", JSON.stringify(result, null, 2));
console.log(JSON.stringify({ passed: result.passed, checks, measurements: result.measurements, audit: `${resultData.audit.passedCount}/${resultData.audit.total}`, pass35: `${resultData.validation.passedCount}/${resultData.validation.total}` }, null, 2));
await browser.close();
server.kill("SIGTERM");
