...
const server = spawn('python3', ['-m', 'http.server', '4190'], { cwd: process.cwd(), stdio: 'ignore' });
...
await page.goto('http://127.0.0.1:4190/index.html', { waitUntil: 'networkidle' });
...
await page.locator('#gameCanvas').screenshot({ path: 'browser-artifacts/pass18-focused-entry.png' });
...
for (let iteration = 0; iteration < 2200; iteration += 1) {
  const state = await page.evaluate(() => window.__corelessV2.debug());
  const p = state.player;
  if (iteration % 5 === 0) samples.push({ iteration, frame: state.frameCount, x: p.x, y: p.y, vx: p.vx, vy: p.vy, grounded: p.grounded, floor: p.standingFloorId, landings: state.progress.pass18NarrowLandings, jumpHeld, jumpSource });
  if (state.resetCount > 0) { failure = `reset at ${p.x.toFixed(1)},${p.y.toFixed(1)}`; break; }
  if (state.progress.pass18Completed) break;
...
await page.locator('#gameCanvas').screenshot({ path: 'browser-artifacts/pass18-focused-exit.png' });

const state = await page.evaluate(() => ({
  title: document.title,
  audit: window.__corelessV2.audit(),
  debug: window.__corelessV2.debug(),
  validation: window.__corelessV2.pass18.validate(),
}));
const checks = {
  title: state.title === 'Coreless · Rebuild V2 · Pass 18',
  runtimeAudit: state.audit.passed && state.audit.passedCount === 27,
  pass17Retained: state.audit.pass17.passed && state.audit.pass17.passedCount === 24,
  pass18Audit: state.validation.passed && state.validation.passedCount === 32,
  entered: state.debug.progress.pass18Entered,
  twelveLandings: state.debug.progress.pass18NarrowLandings === 12,
  shortCuts: state.debug.progress.pass18ShortCuts >= 2,
  heldJumps: state.debug.progress.pass18HeldJumps >= 3,
  noCeilingBumps: state.debug.progress.pass18CeilingBumps === 0,
  completed: state.debug.progress.pass18Completed && state.debug.progress.pass18CheckpointActivated,
...
const result = {
  version: 'rebuild-v2-pass18-focused',
  category: 'focused actual-keyboard traversal after helper placement at the pass 17 final landing',
  helperCoordinateMovement: true,
  actualKeyboardRoute: 'pass 17 final landing -> twelve narrow platforms -> three low-ceiling spans -> lower exit checkpoint',
  failure,
...
fs.writeFileSync('browser-artifacts/pass18-focused-results.json', JSON.stringify(result, null, 2));
