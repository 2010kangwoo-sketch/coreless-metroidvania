...
const server = spawn("python3", ["-m", "http.server", "4220"], { cwd: process.cwd(), stdio: "ignore" });
...
await page.goto("http://127.0.0.1:4220/index.html", { waitUntil: "networkidle" });
...
const sampleMetrics = [];
const samples = await page.evaluate(() => window.__corelessV2.pass33.cameraSamples);
for (const sample of samples) {
  const metrics = await page.evaluate(sampleData => {
    const runtime = window.__corelessV2.runtime;
    const pass33 = window.__corelessV2.pass33;
    ...
    const placeholderColors = [[31, 52, 58], [136, 170, 176], [23, 49, 57], [16, 35, 42], [69, 111, 119]];
    ...
    const scene = pass33.scenes.find(item => item.id === sampleData.sceneId);
    const safety = pass33.plan.renderContract.playerSafetyRect;
    const screenRect = placement => ({
      id: placement.id,
      x: placement.x - (runtime.camera.x - scene.anchorCamera.x) * placement.cameraRatioX,
      y: placement.y - (runtime.camera.y - scene.anchorCamera.y) * placement.cameraRatioY,
      width: placement.width,
      height: placement.height,
    });
    const foregroundOverlapIds = [];
    for (const placement of pass33.placements.filter(item => item.sceneId === scene.id && item.layer === "foreground" && (item.maxCameraX === undefined || runtime.camera.x <= item.maxCameraX))) {
      const rect = screenRect(placement);
      const record = pass33.assetState.byId.get(placement.assetId);
    ...
    const continuationBottoms = pass33.placements.filter(item => item.sceneId === scene.id && item.continuesBelowViewport).map(screenRect).map(rect => ({ id: rect.id, bottom: rect.y + rect.height }));
    return { id: sampleData.id, sceneId: sampleData.sceneId, camera: { ...runtime.camera }, contrastRatio, playerLuminance, backgroundLuminance, flatPlaceholderCoverage: placeholderPixels / measuredPixels, visibleSeamColumns, foregroundOverlapIds, continuationBottoms };
  }, sample);
  sampleMetrics.push(metrics);
  await page.locator("#gameCanvas").screenshot({ path: `browser-artifacts/pass33-sweep-${sample.id}.png` });
}

const assetBottomCoverage = await page.evaluate(() => {
  const result = {};
  for (const id of ["shaft_frame", "uneven_gallery"]) {
    const record = window.__corelessV2.pass33.assetState.byId.get(id);
...
const parallaxProbe = await page.evaluate(() => {
  const pass33 = window.__corelessV2.pass33;
  const scene = pass33.scenes[0];
  const far = pass33.placements.find(item => item.sceneId === scene.id && item.layer === "far_background");
  const foreground = pass33.placements.find(item => item.sceneId === scene.id && item.layer === "foreground");
...
const transitions = [
  { id: "entry_west", cameraX: 10000, cameraY: 3000 }, { id: "entry_mid", cameraX: 10100, cameraY: 3000 }, { id: "entry_east", cameraX: 10200, cameraY: 3000 },
  { id: "compression_to_shaft_west", cameraX: 11160, cameraY: 2980 }, { id: "compression_to_shaft_mid", cameraX: 11250, cameraY: 2980 }, { id: "compression_to_shaft_east", cameraX: 11340, cameraY: 2980 },
  { id: "shaft_to_gallery_west", cameraX: 11710, cameraY: 3100 }, { id: "shaft_to_gallery_mid", cameraX: 11800, cameraY: 3100 }, { id: "shaft_to_gallery_east", cameraX: 11890, cameraY: 3100 },
];
for (const sample of transitions) {
  ...
  await page.locator("#gameCanvas").screenshot({ path: `browser-artifacts/pass33-transition-${sample.id}.png` });
}

await page.keyboard.press("b");
await page.evaluate(() => window.__corelessV2.runtime.__focusedDraw());
await page.locator("#gameCanvas").screenshot({ path: "browser-artifacts/pass33-blueprint-focused.png" });
await page.keyboard.press("b");

const resultData = await page.evaluate(() => ({ title: document.title, buildStatus: document.getElementById("buildStatus")?.textContent ?? null, auditStatus: document.getElementById("auditStatus")?.textContent ?? null, audit: window.__corelessV2.audit(), validation: window.__corelessV2.pass33.validate(), plan: window.__corelessV2.pass33.plan, assetState: { loadedCount: window.__corelessV2.pass33.assetState.loadedCount, failedCount: window.__corelessV2.pass33.assetState.failedCount, dimensionsValid: window.__corelessV2.pass33.assetState.dimensionsValid }, debug: window.__corelessV2.debug() }));
...
const continuationBottoms = sampleMetrics.flatMap(item => item.continuationBottoms);
const minimumContinuationBottom = Math.min(...continuationBottoms.map(item => item.bottom));
...
const checks = {
  title: resultData.title === "Coreless · Rebuild V2 · Pass 33",
  runtimeAudit: resultData.audit.passed && resultData.audit.passedCount === 60,
  pass33Validation: resultData.validation.passed && resultData.validation.passedCount === 61,
  explicitScope: resultData.plan.scope === "uneven_tunnel_and_lift_shaft_only",
  assetsLoaded: resultData.assetState.loadedCount === 6 && resultData.assetState.failedCount === 0,
  assetDimensions: resultData.assetState.dimensionsValid === true,
  nineCameraSamples: sampleMetrics.length === 9,
  threeScenesCovered: new Set(sampleMetrics.map(item => item.sceneId)).size === 3,
  farParallaxSlower: Math.abs(parallaxProbe.farDelta - 8) < 0.01,
  foregroundParallaxFaster: Math.abs(parallaxProbe.foregroundDelta - 110) < 0.01,
...
const result = { version: "rebuild-v2-pass33-focused", category: "uneven tunnel raster depth across nine helper-positioned camera samples; helper movement is not a traversal claim", helperCoordinateMovement: true, passed: Object.values(checks).every(Boolean), checks, measurements: { minimumContrast, maximumFlatCoverage, totalVisibleSeamColumns, foregroundOverlapCount, minimumContinuationBottom, assetBottomCoverage, parallaxProbe, sampleMetrics }, resultData, consoleErrors, pageErrors };
fs.writeFileSync("browser-artifacts/pass33-focused-results.json", JSON.stringify(result, null, 2));
console.log(JSON.stringify({ passed: result.passed, checks, measurements: result.measurements, audit: `${resultData.audit.passedCount}/${resultData.audit.total}`, pass33: `${resultData.validation.passedCount}/${resultData.validation.total}` }, null, 2));
await browser.close();
server.kill("SIGTERM");
