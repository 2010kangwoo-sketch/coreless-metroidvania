...
const server = spawn('python3', ['-m', 'http.server', '4177'], {
  cwd: process.cwd(),
  stdio: 'ignore',
});
...
await page.locator('#gameCanvas').screenshot({ path: 'browser-artifacts/pass09-start.png' });
...
while (loop < 18000) {
  ...
  if (loop % 20 === 0) {
    routeSamples.push({
      ...
      keys: state.keys,
      phase: state.progress.pass09Completed ? 'pass09_complete'
  ...
  if (state.progress.pass09Completed) break;
  ...
  if (firstClimbActive || secondClimbActive) {
    ...
    if (state.progress.pass08Completed && !state.progress.zone07UpperDrop) await setDirection('d');
...
await page.locator('#gameCanvas').screenshot({ path: 'browser-artifacts/pass09-exit.png' });

await page.keyboard.press('b');
await page.waitForTimeout(150);
const blueprintOpened = (await debug()).blueprintVisible === true;
await page.locator('#gameCanvas').screenshot({ path: 'browser-artifacts/pass09-blueprint.png' });
await page.keyboard.press('b');
await page.waitForTimeout(150);
...
const deterministicChecks = {
  title: state.title === 'Coreless · Rebuild V2 · Pass 09',
  canvas: state.canvas?.width === 1200 && state.canvas?.height === 680,
  focused: state.activeElement === 'gameCanvas',
  runtimeAudit: state.audit?.passed === true && state.audit?.passedCount === 18,
  ...
  collapseBehindPlayer: (state.debug?.progress?.floorsCollapsed ?? 0) >= 36,
  supportsDestroyed: (state.debug?.progress?.supportsDestroyed ?? 0) >= 15,
  boulderCoveredRoute: (state.debug?.chase?.pathProgress ?? 0) >= 0.90,
  ...
  wallJumps: (state.debug?.progress?.wallJumps ?? 0) >= 2,
...
const result = {
  version: 'rebuild-v2-pass09',
  testedWith: 'Chromium + Playwright actual keyboard events',
  actualKeyboardRoute: 'start slope -> active chase -> giant curve -> pass 08 boundary -> upper internal run -> drop -> middle return -> second drop -> lower internal run -> pass 09 exit',
  ...
  pageErrors,
  limitations: [
    'Only zones 01 through 07 have playable collision in pass 09.',
    'The remaining three zones are still blueprint data.',
    'The active chase currently seals at the Pass 09 exit; the bridge finale is not implemented.',
...
fs.writeFileSync('browser-artifacts/pass09-results.json', JSON.stringify(result, null, 2));
