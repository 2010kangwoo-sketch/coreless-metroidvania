...
const server = spawn("python3", ["-m", "http.server", "4244"], { cwd: process.cwd(), stdio: "ignore" });
...
await page.goto("http://127.0.0.1:4244/index.html", { waitUntil: "networkidle" });
...
const samples = await page.evaluate(() => window.__corelessV2.pass39.cameraSamples);
const sampleMetrics = [];
for (const sample of samples) {
  const metrics = await page.evaluate(sampleData => {
    const runtime = window.__corelessV2.runtime;
    const pass39 = window.__corelessV2.pass39;
    Object.assign(runtime.progress, {
      pass06Completed: true,
      zone06Entered: true,
      curveCommitted: true,
      zone07Entered: true,
      zone07FirstLanding: true,
      zone07MiddleTurn: true,
      zone07SecondLanding: true,
    ...
    Object.assign(runtime.camera, { x: sampleData.cameraX, y: sampleData.cameraY, zoom: sampleData.cameraZoom ?? 1 });
    runtime.__focusedDraw();
    const canvas = document.getElementById("gameCanvas");
    const context = canvas.getContext("2d", { willReadFrequently: true });
    const pixels = context.getImageData(0, 0, canvas.width, canvas.height);
    const at = (x, y) => {
      const safeX = Math.max(0, Math.min(pixels.width - 1, x));
      const safeY = Math.max(0, Math.min(pixels.height - 1, y));
      const index = (safeY * pixels.width + safeX) * 4;
    ...
    const zoom = sampleData.cameraZoom ?? 1;
    const screenPlayerX = Math.round((sampleData.playerX - sampleData.cameraX) * zoom);
    const screenPlayerY = Math.round((sampleData.playerY - sampleData.cameraY) * zoom);
    const playerPixels = [];
    for (let y = screenPlayerY + 4; y < screenPlayerY + Math.max(12, 43 * zoom); y += 2) for (let x = screenPlayerX + 4; x < screenPlayerX + Math.max(10, 27 * zoom); x += 2) playerPixels.push(at(x, y));
    ...
    const scene = pass39.scenes.find(item => item.id === sampleData.sceneId);
    ...
    const continuationBottoms = pass39.placements.filter(item => item.sceneId === scene.id && item.continuesBelowViewport).map(screenRect).map(rect => ({ id: rect.id, bottom: rect.y + rect.height }));
  ...
  await page.locator("#gameCanvas").screenshot({ path: `browser-artifacts/pass39-sweep-${sample.id}.png` });
}

const assetBottomCoverage = await page.evaluate(() => {
  const result = {};
  for (const id of ["bridge_entry", "bridge_mid", "bridge_final", "aftershock_entry", "aftershock_mid", "aftershock_exit"]) {
    const record = window.__corelessV2.pass39.assetState.byId.get(id);
    const canvas = document.createElement("canvas");
    canvas.width = record.width;
    canvas.height = 20;
    const context = canvas.getContext("2d", { willReadFrequently: true });
    context.drawImage(record.image, 0, record.height - 20, record.width, 20, 0, 0, record.width, 20);
    const alpha = context.getImageData(0, 0, record.width, 20).data;
    let covered = 0;
    for (let index = 3; index < alpha.length; index += 4) if (alpha[index] > 32) covered += 1;
    result[id] = covered / (record.width * 20);
  }
  return result;
});
...
const parallaxProbe = await page.evaluate(() => {
  const pass39 = window.__corelessV2.pass39;
  const scene = pass39.scenes[0];
  const far = pass39.placements.find(item => item.sceneId === scene.id && item.layer === "far_background");
  const foreground = pass39.placements.find(item => item.sceneId === scene.id && item.layer === "foreground");
...
await page.locator("#gameCanvas").screenshot({ path: "browser-artifacts/pass39-blueprint-focused.png" });
await page.keyboard.press("b");

const resultData = await page.evaluate(() => ({ title: document.title, audit: window.__corelessV2.audit(), validation: window.__corelessV2.pass39.validate(), plan: window.__corelessV2.pass39.plan, assetState: { loadedCount: window.__corelessV2.pass39.assetState.loadedCount, failedCount: window.__corelessV2.pass39.assetState.failedCount, dimensionsValid: window.__corelessV2.pass39.assetState.dimensionsValid }, debug: window.__corelessV2.debug() }));
...
const checks = {
  title: resultData.title === "Coreless · Rebuild V2 · Pass 39",
  runtimeAudit: resultData.audit.passed && resultData.audit.passedCount === 90,
  pass39Validation: resultData.validation.passed && resultData.validation.passedCount === 48,
  explicitScope: resultData.plan.scope === "collapsing_bridge_finale_and_aftershock_precision_only",
  assetsLoaded: resultData.assetState.loadedCount === 10 && resultData.assetState.failedCount === 0,
  assetDimensions: resultData.assetState.dimensionsValid === true,
  twelveCameraSamples: sampleMetrics.length === 12,
  sixScenesCovered: new Set(sampleMetrics.map(item => item.sceneId)).size === 6,
  ...
  noMagentaLeak: maximumMagentaLeakCoverage <= resultData.plan.thresholds.maximumChromaLeakRatio,
  ...
  bridgeAftershockComposite: resultData.plan.sceneCount === 6 && resultData.plan.placementCount === 24,
...
const result = { version: "rebuild-v2-pass39-focused", category: "collapsing bridge finale and aftershock precision raster depth across twelve helper-positioned camera samples; helper movement is not a traversal claim", helperCoordinateMovement: true, passed: Object.values(checks).every(Boolean), checks, measurements: { minimumContrast, maximumFlatCoverage, maximumMagentaLeakCoverage, minimumContinuationBottom, assetBottomCoverage, parallaxProbe, sampleMetrics }, resultData, consoleErrors, pageErrors };
fs.writeFileSync("browser-artifacts/pass39-focused-results.json", JSON.stringify(result, null, 2));
console.log(JSON.stringify({ passed: result.passed, checks, measurements: result.measurements, audit: `${resultData.audit.passedCount}/${resultData.audit.total}`, pass39: `${resultData.validation.passedCount}/${resultData.validation.total}` }, null, 2));
await browser.close();
server.kill("SIGTERM");
