...
const server = spawn("python3", ["-m", "http.server", "4236"], { cwd: process.cwd(), stdio: "ignore" });
...
await page.goto("http://127.0.0.1:4236/index.html", { waitUntil: "networkidle" });
...
const samples = await page.evaluate(() => window.__corelessV2.pass36.cameraSamples);
const sampleMetrics = [];
for (const sample of samples) {
  const metrics = await page.evaluate(sampleData => {
    const runtime = window.__corelessV2.runtime;
    const pass36 = window.__corelessV2.pass36;
    ...
    const scene = pass36.scenes.find(item => item.id === sampleData.sceneId);
    ...
    const continuationBottoms = pass36.placements.filter(item => item.sceneId === scene.id && item.continuesBelowViewport).map(screenRect).map(rect => ({ id: rect.id, bottom: rect.y + rect.height }));
  ...
  await page.locator("#gameCanvas").screenshot({ path: `browser-artifacts/pass36-sweep-${sample.id}.png` });
}

const assetBottomCoverage = await page.evaluate(() => {
  const result = {};
  for (const id of ["internal_descent", "internal_lower", "shaft_one", "shaft_two", "lower_hall", "exit_rise"]) {
    const record = window.__corelessV2.pass36.assetState.byId.get(id);
...
const parallaxProbe = await page.evaluate(() => {
  const pass36 = window.__corelessV2.pass36;
  const scene = pass36.scenes[0];
  const far = pass36.placements.find(item => item.sceneId === scene.id && item.layer === "far_background");
  const foreground = pass36.placements.find(item => item.sceneId === scene.id && item.layer === "foreground");
...
await page.locator("#gameCanvas").screenshot({ path: "browser-artifacts/pass36-blueprint-focused.png" });
await page.keyboard.press("b");

const resultData = await page.evaluate(() => ({ title: document.title, audit: window.__corelessV2.audit(), validation: window.__corelessV2.pass36.validate(), plan: window.__corelessV2.pass36.plan, assetState: { loadedCount: window.__corelessV2.pass36.assetState.loadedCount, failedCount: window.__corelessV2.pass36.assetState.failedCount, dimensionsValid: window.__corelessV2.pass36.assetState.dimensionsValid }, debug: window.__corelessV2.debug() }));
...
const checks = {
  title: resultData.title === "Coreless · Rebuild V2 · Pass 36",
  runtimeAudit: resultData.audit.passed && resultData.audit.passedCount === 75,
  pass36Validation: resultData.validation.passed && resultData.validation.passedCount === 46,
  explicitScope: resultData.plan.scope === "internal_descent_and_double_wall_chase_only",
  assetsLoaded: resultData.assetState.loadedCount === 9 && resultData.assetState.failedCount === 0,
  assetDimensions: resultData.assetState.dimensionsValid === true,
  fifteenCameraSamples: sampleMetrics.length === 15,
  nineScenesCovered: new Set(sampleMetrics.map(item => item.sceneId)).size === 9,
  ...
  fourRasterWallFaces: resultData.plan.wallFaceCount === 4,
...
const result = { version: "rebuild-v2-pass36-focused", category: "internal descent and double-wall chase raster depth across fifteen helper-positioned camera samples; helper movement is not a traversal claim", helperCoordinateMovement: true, passed: Object.values(checks).every(Boolean), checks, measurements: { minimumContrast, maximumFlatCoverage, maximumMagentaLeakCoverage, minimumContinuationBottom, assetBottomCoverage, parallaxProbe, sampleMetrics }, resultData, consoleErrors, pageErrors };
fs.writeFileSync("browser-artifacts/pass36-focused-results.json", JSON.stringify(result, null, 2));
console.log(JSON.stringify({ passed: result.passed, checks, measurements: result.measurements, audit: `${resultData.audit.passedCount}/${resultData.audit.total}`, pass36: `${resultData.validation.passedCount}/${resultData.validation.total}` }, null, 2));
await browser.close();
server.kill("SIGTERM");
