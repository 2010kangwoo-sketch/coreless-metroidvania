...
const verifyPass40 = targetPass >= 40;
...
const deterministicChecks = {
  ...
  pass37Audit: !verifyPass37 || (state.audit?.pass37?.passed === true && state.audit?.pass37?.passedCount === 44),
  pass37AssetsLoaded: !verifyPass37 || (state.audit?.pass37Assets?.loadedCount === 8 && state.audit?.pass37Assets?.failedCount === 0),
  pass37AssetDimensions: !verifyPass37 || state.audit?.pass37Assets?.dimensionsValid === true,
  pass37ExplicitScope: !verifyPass37 || state.audit?.pass37?.checks?.some(check => check.id === 'scope_explicit' && check.passed === true),
  pass37ParallaxContract: !verifyPass37 || state.audit?.pass37?.checks?.some(check => check.id === 'visible_parallax_separation' && check.passed === true),
  pass37SupportContinuation: !verifyPass37 || state.audit?.pass37?.checks?.some(check => check.id === 'supports_extend_below_viewport' && check.passed === true),
  pass37GrappleRasterContract: !verifyPass37 || state.audit?.pass37?.checks?.some(check => check.id === 'grapple_sprite_exists' && check.passed === true),
  pass37ZeroCollisionChanges: !verifyPass37 || state.audit?.pass37?.checks?.some(check => check.id === 'zero_collision_changes' && check.passed === true),
  pass38Audit: !verifyPass38 || (state.audit?.pass38?.passed === true && state.audit?.pass38?.passedCount === 45),
  pass38AssetsLoaded: !verifyPass38 || (state.audit?.pass38Assets?.loadedCount === 8 && state.audit?.pass38Assets?.failedCount === 0),
  pass38AssetDimensions: !verifyPass38 || state.audit?.pass38Assets?.dimensionsValid === true,
  pass38ExplicitScope: !verifyPass38 || state.audit?.pass38?.checks?.some(check => check.id === 'scope_explicit' && check.passed === true),
  pass38ParallaxContract: !verifyPass38 || state.audit?.pass38?.checks?.some(check => check.id === 'visible_parallax_separation' && check.passed === true),
  ...
  pass40Audit: !verifyPass40 || (state.audit?.pass40?.passed === true && state.audit?.pass40?.passedCount === 32),
  pass40DirectEntry: !verifyPass40 || state.audit?.pass40?.checks?.some(check => check.id === 'direct_entry_declared' && check.passed === true),
  pass40RuntimeBudget: !verifyPass40 || state.audit?.pass40?.checks?.some(check => check.id === 'runtime_asset_budget' && check.passed === true),
  pass40RuntimeReduction: !verifyPass40 || state.audit?.pass40?.checks?.some(check => check.id === 'runtime_asset_reduction' && check.passed === true),
  pass40WebpRuntime: !verifyPass40 || state.audit?.checks?.some(check => check.id === 'pass40_runtime_assets_webp' && check.passed === true),
  pass40ReadyState: !verifyPass40 || (
    state.audit?.checks?.some(check => check.id === 'pass40_document_ready' && check.passed === true)
    && state.audit?.checks?.some(check => check.id === 'pass40_canvas_ready' && check.passed === true)
    && state.audit?.checks?.some(check => check.id === 'pass40_loading_surface_released' && check.passed === true)
  ),
...
const result = {
  ...
  actualKeyboardRoute: verifyPass40
    ? 'DIRECT SITE ENTRY -> automatic optimized asset load -> focused canvas -> START rooted sanctuary -> complete forty-pass mega-room route -> final late checkpoint GOAL'
    : verifyPass39
  ...
  pageErrors,
  limitations: [
    verifyPass40 ? 'Pass 40 completes the planned integration and direct-entry release gate; subtle blur, shape simplification, and character-environment style unification remain intentionally deferred to the post-40 visual polish phase.' :
